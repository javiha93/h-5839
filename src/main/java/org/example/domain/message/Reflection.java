...
public class Reflection {
    ...
    public Object getFieldValue(MessagePosition messagePosition) {
        try {
            String fieldName = messagePosition.getValue();
            String methodName = "get" + capitalize(fieldName);

            Method method = getClass().getMethod(methodName);

            if (messagePosition.isLastPosition()) {
                return method.invoke(this);
            } else {
                Reflection object = getNestedObject(method, methodName);
                return object.getFieldValue(messagePosition.getChildMessagePosition());
            }
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public void setFieldValue(MessagePosition messagePosition, String value) {
        if (value == null) {
            return;
        }
        try {
            ...
            if (messagePosition.isLastPosition()) {
                ...
                Method method = getClass().getMethod(methodName, value.getClass());
                method.invoke(this, value);
            } else {
                methodName = "get" + capitalize(fieldName);
                Reflection object = getNestedObject(getClass().getMethod(methodName), methodName);
                if (methodName.equals("getSupplementalInfos")) {
                    ((SupplementalInfoList) object).manageSupplementalInfo(messagePosition.getChildMessagePosition(), value);
                    return;
                }
                object.setFieldValue(messagePosition.getChildMessagePosition(), value);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
